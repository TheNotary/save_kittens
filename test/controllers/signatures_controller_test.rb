...
class SignaturesControllerTest < ActionController::TestCase
  ...
  test "should create signature" do
    assert_difference('Signature.count') do
      post :create, signature: { email: @signature.email, name: @signature.name, state: @signature.state, subscriber: @signature.subscriber, zip: @signature.zip }
    end
  ...
  test "should update signature" do
    patch :update, id: @signature, signature: { email: @signature.email, name: @signature.name, state: @signature.state, subscriber: @signature.subscriber, zip: @signature.zip }
    assert_redirected_to signature_path(assigns(:signature))
  end
